...
SVG;
    ...
    {   
        ...
        if(is_null($ticket))
        {
            //get ticket id from uri
            $ticketId = explode('/',$request->getRequestUri())[5]; 
            
            $ticket = $this->entityManager->getRepository('UVDeskCoreFrameworkBundle:Ticket')->findOneById($ticketId);

        }

        // for customer, ticketId is null, ticket is null
		
		if ($this->userService->getCurrentUser()->getCurrentInstance()->getSupportRole()->getCode() === 'ROLE_CUSTOMER') {
			return $this->twig->render('@_uvdesk_extension_uvdesk_custom_fields\widgets\CustomFields\customFieldSnippetCustomer.html.twig', 
                        $this->customFieldsService->getCustomerCustomFieldSnippet($ticket));
        } else {
			return $this->twig->render('@_uvdesk_extension_uvdesk_custom_fields\widgets\CustomFields\customFieldSnippet.html.twig', 
